...
export async function POST(req, { params }) {
  try {
    ...
    const team = await Event2.findOne({ teamCode: teamCode });
    if (!team) {
    ...
    const Event2TeamToken = await event2TeamToken.findOne({ teamId: team._id });
    if (!Event2TeamToken) {
    ...
    //     });
    // }
    if (teamCode !== Event2TeamToken.token) {
      return NextResponse.json({ error: "Incorrect token" }, {status: 506});
    }
    await Users.findOneAndUpdate(
      { _id: userId },
      { $set: { event2TeamId: team.id, event2TeamRole: 1 } }
    );
