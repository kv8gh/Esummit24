    ...
    try{
       ...
       // console.log(userId);
        ...
        if (!team.teamCode) {
            ...
            await TeamModel.findOneAndUpdate({ _id: team._id }, { $set: { teamCode: teamCode } });

           
            return NextResponse.json({"teamCode": teamCode,
            ...
            if (timeDifference > 10) {
                ...
                await TeamToken.findOneAndUpdate({ teamId: team._id }, { $set: { token: newTeamCode, createdAt: currentTime } });
                await TeamModel.findOneAndUpdate({ _id: team._id }, { $set: { teamCode: newTeamCode } });

                return NextResponse.json({  "teamCode": newTeamCode, "teamName": team.teamName});
            } else {
                return NextResponse.json({  "teamCode": token.token, "teamName": team.teamName});
